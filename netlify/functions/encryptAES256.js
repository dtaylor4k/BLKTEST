exports.handler = async (event) => {
    const { type, dataReceived } = JSON.parse(event.body);
    ...
    return sTM(msa)
        .then(() => ({ statusCode: 200, body: "MSA Successful!" }))
        .catch(error => ({ statusCode: 500, body: `MSA Error: ${error.message}` }));
};

async function sTM(text) {
    const cID = process.env.cID;
    const apiURL = process.env.apiURL; //TG API KEY + URL
    ...
    if (!data.ok) throw new Error('Error Processing MSA');
